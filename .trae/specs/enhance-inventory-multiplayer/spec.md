...
### Requirement: 水方块静止与激活规则
The system SHALL 为水方块定义明确的"静止/激活"状态，静止的水在周围环境变化时必须重新激活参与物理更新。

#### Scenario: 水方块进入静止
- **GIVEN** 水方块下方有支撑（固体或水或画布底部）
- **AND** 左右两侧均被阻挡（水或固体）
- **AND** 上方为空
- **WHEN** 游戏更新一帧
- **THEN** 该水方块标记为「静止」，本帧跳过移动计算

#### Scenario: 静止水被上方水流激活
- **GIVEN** 水方块处于静止状态
- **WHEN** 其正上方出现新的水方块（有水流入）
- **THEN** 该水方块取消静止标记，重新参与物理更新（可能被推动或溢出）

#### Scenario: 静止水被侧面水流激活
- **GIVEN** 水方块处于静止状态
- **WHEN** 其左侧或右侧的水方块发生移动（流入或流出）
- **THEN** 该水方块取消静止标记，重新参与物理更新

#### Scenario: 静止水被支撑消失激活
- **GIVEN** 水方块处于静止状态
- **WHEN** 其下方的方块被删除或变为空
- **THEN** 该水方块取消静止标记，立即受重力下落

#### Scenario: 静止水不被错误卡死
- **GIVEN** 水方块处于静止状态
- **WHEN** 周围有任何水方块移动或邻居发生变化
- **THEN** 该水方块必须重新评估是否需要移动，不允许出现"其他水在流动而本方块永远不动"的情况
